...
class CPUView:
    ...
    def incrementCycles(self): # if this is called, it should reprint the scoreboard. should be called at the end of each clock cycle
        self.cycles += 1
        self.printScoreboard()
        pass
    
    def printScoreboard(self): # called each clock cycle
        os.system('cls' if os.name == 'nt' else 'clear') # clears scoreboard so it can be updated, probably needs to be changed in some way
